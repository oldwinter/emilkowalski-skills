...
这是 `write-swift` 的中文 runtime 入口。编写、审查或迁移 Swift 时，先以 value types 和最简单的单线程实现为默认，再按证据逐步引入 error model、Swift 6 concurrency、`Sendable`、structured concurrency、protocols/generics、ARC 优化、Swift Testing、macros 与 unsafe 能力。遇到 data race、hang、retain cycle 或性能问题时，沿下方决策顺序定位根因，不要跳过 profiling、toolchain baseline 和 isolation 约束。保留 Swift 标识符、代码、版本号、build settings、命令和精确 API 名称原样。
